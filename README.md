...
1. __Branches__: The branches aside from the `main` branch are used to store different parts of the tutorial. Each branch contains a specific stage in the process. For example, the `lesson_1` branch contains all scripts listed in the 1st lesson, etc.
...
- __info.txt__: A file that enables the bot running in the action to detect the topic of the lesson identified by the branch name.It should consist of a topic name and a description of the topic, in a key-value as format shown below, but can also contain other optional metadata:
